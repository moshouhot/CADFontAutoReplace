...
internal static class PluginUninstaller
{
    ...
    {
        warningMessage = null;
        // 1. 按 __Owned 标记清理外部键（FixedProfile 等），仅在标记值与现值一致时清除。
        try
        {
        ...
        // 2. 仅删除本插件的 Applications\AppName 子键，禁止删除 Applications 父项或其它插件项。
        try
        {
            foreach (var profileSubKey in installation.ProfileSubKeys)
            {
